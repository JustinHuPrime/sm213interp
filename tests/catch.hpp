...
/// visible - but it does mean (substring) StringRefs should not be shared
/// between threads.
class StringRef {
 public:
  ...
  do {                                                                      \
    ...
    INTERNAL_CATCH_REACT(catchAssertionHandler)                             \
           ...
           false && static_cast<bool>(                                      \
                        !!(__VA_ARGS__)))  // the expression here is never
                                           // evaluated at runtime but it forces
...
class BenchmarkLooper {
 ...
 public:
  ...
  explicit operator bool() {
    if (m_count < m_iterationsToRun) return true;
...
#define CATCH_INTERNAL_ERROR(msg)                 \
  Catch::throw_exception(CATCH_PREPARE_EXCEPTION( \
      std::logic_error,                           \
      CATCH_INTERNAL_LINEINFO << ": Internal Catch error: " << msg))
...
class SingleValueGenerator : public IGenerator<T> {
 ...
 public:
  ...
  auto get(size_t) const -> T override { return m_value; }
};
...
class FixedValuesGenerator : public IGenerator<T> {
 ...
 public:
  ...
  auto get(size_t index) const -> T override { return m_values[index]; }
};
...
class Generator {
 ...
 public:
  Generator(size_t size, std::unique_ptr<IGenerator<T>> generator)
      : m_generator(std::move(generator)), m_size(size) {}
...
auto range(T const& first, T const& last) -> Generator<T> {
  return Generator<T>((last - first),
                      pf::make_unique<RangeGenerator<T>>(first, last));
}
...
auto random(T const& first, T const& last) -> Generator<T> {
  ...
  return Generator<T>(
      size, pf::make_unique<GeneratorRandomiser<T>>(std::move(gen), size));
}
template <typename T>
auto random(size_t size) -> Generator<T> {
  return Generator<T>(size,
                      pf::make_unique<GeneratorRandomiser<T>>(all<T>(), size));
}

template <typename T>
auto values(std::initializer_list<T> values) -> Generator<T> {
  return Generator<T>(values.size(),
...
auto table(std::initializer_list<std::tuple<Ts...>>&& tuples)
    -> Generator<std::tuple<Ts...>> {
  return values<std::tuple<Ts...>>(
      std::forward<std::initializer_list<std::tuple<Ts...>>>(tuples));
}
...
struct Generators : GeneratorBase {
  ...
  auto operator[](size_t index) const -> T {
    ...
    for (auto const& gen : m_generators) {
      ...
      sizes += gen.size();
      if (index < sizes) return gen[localIndex];
    }
...
auto makeGenerators(Generator<T>&& generator, Gs... moreGenerators)
    -> Generators<T> {
...
auto generate(SourceLineInfo const& lineInfo, L const& generatorExpression)
  ...
  auto const& generator =
      static_cast<Generators<UnderlyingType> const&>(*tracker.getGenerator());
...
void Approx::setMargin(double margin) {
  CATCH_ENFORCE(margin >= 0, "Invalid Approx::margin: "
                                 << margin << '.'
...
auto BenchmarkLooper::needsMoreIterations() -> bool {
  ...
  // Exponentially increasing iterations until we're confident in our timer
  // resolution
...
clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto const setWaitForKeypress = [&](std::string const& keypress) {
    ...
    if (keypressLc == "start")
      ...
      return ParserResult::runtimeError(
          "keypress argument must be one of: start, exit or both. '" +
...
Config::Config(ConfigData const& data) : m_data(data), m_stream(openStream()) {
  ...
  if (data.testsOrTags.empty()) {
    ...
    for (auto const& testOrTags : data.testsOrTags) parser.parse(testOrTags);
  }
...
void FatalConditionHandler::reset() {
  if (isSet) {
    // Set signals back to previous values -- hopefully nobody overwrote them in
    // the meantime
    for (std::size_t i = 0; i < sizeof(signalDefs) / sizeof(SignalDefs); ++i) {
...
template <>
auto all<int>() -> Generator<int> {
  return range(std::numeric_limits<int>::min(),
...
std::string TagInfo::all() const {
  std::string out;
  for (auto const& spelling : spellings) out += "[" + spelling + "]";
  return out;
}
...
TempFile::TempFile() {
  ...
  if (fopen_s(&m_file, m_buffer, "w")) {
    ...
    CATCH_RUNTIME_ERROR("Coul dnot open the temp file: '"
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
  ...
  auto getIndex() const -> size_t override { return m_index; }
};
...
int Session::run(int argc, wchar_t* const argv[]) {
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, utf8Argv[i], bufSize, NULL,
                        NULL);
  }
...
auto StringRef::c_str() const -> char const* {
  if (isSubstring()) const_cast<StringRef*>(this)->takeOwnership();
  return m_start;
}
...
auto StringRef::numberOfCharacters() const noexcept -> size_type {
  ...
  for (size_type i = 0; i < m_size; ++i) {
    ...
    if ((c & byte_2_lead) == byte_2_lead) {
      noChars--;
      if ((c & byte_3_lead) == byte_3_lead) noChars--;
      if ((c & byte_4_lead) == byte_4_lead) noChars--;
    }
  }
  return noChars;
}
...
void enforceNotReservedTag(std::string const& tag,
  ...
  CATCH_ENFORCE(!isReservedTag(tag),
                "Tag name: [" << tag << "] is not allowed.\n"
                              << "Tag names starting with non alpha-numeric "
...
std::vector<TestCase> filterTests(std::vector<TestCase> const& testCases,
  ...
  for (auto const& testCase : testCases)
    if (matchTest(testCase, testSpec, config)) filtered.push_back(testCase);
  return filtered;
}
std::vector<TestCase> const& getAllTestCasesSorted(IConfig const& config) {
...
bool TestSpec::matches(TestCaseInfo const& testCase) const {
  ...
  for (auto const& filter : m_filters)
    if (filter.matches(testCase)) return true;
  return false;
}
}  // namespace Catch
...
XmlWriter::ScopedElement& XmlWriter::ScopedElement::writeText(
    std::string const& text, bool indent) {
...
ConsoleReporter::ConsoleReporter(ReporterConfig const& config)
    : StreamingReporterBase(config),
      m_tablePrinter(new TablePrinter(
          config.stream(), {{"benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32,
                             ColumnInfo::Left},
...
void ConsoleReporter::benchmarkStarting(BenchmarkInfo const& info) {
  ...
  auto nameCol = Column(info.name).width(
